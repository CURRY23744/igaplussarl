        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4">
                    <form action="{{ route('admin.realisations.update', $realisation->id) }}" method="POST" enctype="multipart/form-data" id="realisationForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description"
                                      ...
                                      placeholder="Décrivez votre réalisation...">{{ old('description', $realisation->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-white border-0 px-4 py-3 d-flex align-items-center justify-content-between">
                    <h5 class="mb-0 fw-semibold">
                        ...
                        Photos existantes
                        <span class="badge bg-primary bg-opacity-10 text-primary ms-2 rounded-pill">{{ $realisation->images->count() }}</span>
                    </h5>
                </div>
                <div class="card-body px-4">
                    <div class="preview-grid">
                        @foreach($realisation->images as $image)
                            <div class="preview-item" id="img-{{ $image->id }}">
                                <img src="{{ asset('storage/' . $image->image_path) }}" alt="">
                                <form action="{{ route('admin.realisations.deleteImage', $image->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="remove-btn" onclick="return confirm('Supprimer cette image ?')">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4">
                    <div class="upload-area border rounded-4 p-5 text-center bg-light bg-opacity-25 mb-4" id="uploadArea">
                        <input type="file" name="images[]" id="images" multiple accept="image/*" class="d-none" form="realisationForm">
            ...
            <!-- Documents existants -->
            @if($realisation->documents->isNotEmpty())
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-white border-0 px-4 py-3">
                    <h5 class="mb-0 fw-semibold">
                        <i class="fas fa-paperclip me-2 text-primary"></i>
                        Documents existants
                        <span class="badge bg-primary bg-opacity-10 text-primary ms-2 rounded-pill">{{ $realisation->documents->count() }}</span>
                    </h5>
                </div>
                <div class="card-body px-4">
                    <div class="docs-list">
                        @foreach($realisation->documents as $document)
                            @php
                                ...
                                if ($mime === 'application/pdf' || $ext === 'pdf') {
                                    $iconCls = 'pdf'; $iconFa = 'fa-file-pdf';
                                } elseif (str_contains($mime, 'word') || in_array($ext, ['doc','docx'])) {
                                    $iconCls = 'word'; $iconFa = 'fa-file-word';
                                } elseif (str_contains($mime, 'excel') || str_contains($mime, 'spreadsheet') || in_array($ext, ['xls','xlsx'])) {
                                    $iconCls = 'excel'; $iconFa = 'fa-file-excel';
                                } elseif (str_starts_with($mime, 'image/')) {
                                    $iconCls = 'image'; $iconFa = 'fa-file-image';
                                } else {
                                    $iconCls = 'other'; $iconFa = 'fa-file-alt';
                                }

                                $size = $document->file_size
                                    ? ($document->file_size < 1048576
                                        ? round($document->file_size / 1024, 1) . ' Ko'
                                        : round($document->file_size / 1048576, 1) . ' Mo')
                                    : '';
                            @endphp
                            <div class="doc-item" id="doc-{{ $document->id }}">
                                <div class="doc-icon {{ $iconCls }}">
                                    <i class="fas {{ $iconFa }}"></i>
                                </div>
                                <div class="doc-info">
                                    <div class="doc-name" title="{{ $name }}">{{ $name }}</div>
                                    <div class="doc-size d-flex align-items-center gap-2">
                                        @if($size)<span>{{ $size }}</span>@endif
                                        <a href="{{ asset('storage/' . $document->file_path) }}" target="_blank" class="text-primary text-decoration-none" style="font-size:0.75rem;">
                                            <i class="fas fa-external-link-alt me-1"></i>Voir
                                        </a>
                                    </div>
                                </div>
                                <form action="{{ route('admin.realisations.deleteDocument', $document->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="doc-remove" title="Supprimer" onclick="return confirm('Supprimer ce document ?')">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </form>
                            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4">
                    <div class="upload-area border rounded-4 p-5 text-center bg-light bg-opacity-25 mb-3" id="docsUploadArea">
                        <input type="file" name="documents[]" id="documents" multiple class="d-none" form="realisationForm">
                ...
                <div class="card-body px-4">
                    <div class="mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="small fw-medium text-dark" id="previewTitre">{{ $realisation->titre }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="small text-secondary">Description</span>
                            <span class="small fw-medium text-dark" id="previewDesc">
                                {{ \Illuminate\Support\Str::limit($realisation->description, 30) ?: 'Non définie' }}
                            </span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="small text-secondary">Photos existantes</span>
                            <span class="small fw-medium text-dark">{{ $realisation->images->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="small text-secondary">Nouvelles photos</span>
                            <span class="small fw-medium text-dark" id="previewCount">0</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="small text-secondary">Docs existants</span>
                            <span class="small fw-medium text-dark">{{ $realisation->documents->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span class="small text-secondary">Nouveaux docs</span>
...
<style>
    body { background-color: #f8f9fc; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }

    .form-control, .form-select { border-color: #e9ecef; transition: all 0.2s ease; }
    ...
    .upload-area:hover { border-color: #0d6efd !important; background-color: rgba(13,110,253,0.02) !important; }
    .upload-area.highlight { border-color: #0d6efd !important; background-color: rgba(13,110,253,0.05) !important; }
    ...
    .preview-item .remove-btn {
        ...
        border: none; color: #dc3545; font-size: 10px;
    ...
    .doc-name { font-size: 0.85rem; font-weight: 500; color: #212529; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .doc-size { font-size: 0.75rem; color: #6c757d; }
    .doc-remove {
    ...
    @media (max-width: 992px) { .sticky-top { position: static; } }
</style>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        uploadArea.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files.length) { imagesInput.files = e.dataTransfer.files; handleImages(e.dataTransfer.files); }
        });
            ...
            Array.from(files).forEach((file, index) => {
                ...
                reader.onload = function (e) {
                    ...
                    item.querySelector('.remove-btn').addEventListener('click', function (ev) {
                        ev.stopPropagation(); item.remove();
        ...
        docsUploadArea.addEventListener('drop', (e) => {
            e.preventDefault(); docsUploadArea.classList.remove('highlight');
            if (e.dataTransfer.files.length) { documentsInput.files = e.dataTransfer.files; handleDocs(e.dataTransfer.files); }
        });
        ...
        function getDocIcon(file) {
            ...
            if (type === 'application/pdf') return { cls: 'pdf', icon: 'fa-file-pdf' };
            if (type.includes('word') || name.endsWith('.doc') || name.endsWith('.docx')) return { cls: 'word', icon: 'fa-file-word' };
            if (type.includes('excel') || type.includes('spreadsheet') || name.endsWith('.xls') || name.endsWith('.xlsx')) return { cls: 'excel', icon: 'fa-file-excel' };
            if (type.startsWith('image/')) return { cls: 'image', icon: 'fa-file-image' };
        ...
        function handleDocs(files) { docsFiles = Array.from(files); renderDocs(); }
        ...
        function renderDocs() {
            ...
            docsFiles.forEach((file, index) => {
                ...
                item.innerHTML = `
                    ...
                    <button type="button" class="doc-remove" title="Supprimer"><i class="fas fa-times"></i></button>`;
